...
cd scripts/ubuntu
for H in $UBU; do
  for S in 01-base.sh 02-ufw.sh 03-ufw-deny-outgoing.sh 04-sudolog-canonical.sh 05-sudolog-main-sudoers.sh 06-grub-backlog-layout.sh; do
...
`sudo -n true` in a fresh session to be sure sudoers is still good. 06 must print
"effective grub config identical", if it says DIFFERENT it already rolled itself back.
